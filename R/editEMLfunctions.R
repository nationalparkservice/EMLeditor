...
#' emlObject<-set_title(emlObject, DataPackageTitle)
#' }
set_title<-function(emlObject, DataPackageTitle, NPS=TRUE){
  ...
  if(NPS==TRUE){
    emlObject<-.set_npspublisher(emlObject)
  }
  #add/update EMLeditor and version to metadata:
  emlObject<-.set_version(emlObject)
  return(emlObject)
  }
...
#' @param emlObject is an R object imported (typically from an EML-formatted .xml file) using EmL::read_eml(<filename>, from="xml").
#'
#' @param DSref is the same as the 7-digit reference code generated on DataStore when the draft reference is initiated. Don't worry about the https://www.doi.org and the data package prefix - those will all automatically be added in by the function.
#'
...
#' @examples
#'  \dontrun{
#'  emlObject<-new_doi(emlObject, "1111111")
#' }
new_doi<-function(emlObject, DSref, NPS=TRUE){
  emlObject$dataset$alternateIdentifier<-paste0("doi: https://doi.org/10.57830/", DSref)
...
#' set_park_units(emlObject, ParkUnits)
#' }
set_park_units<-function(emlObject, ParkUnits, NPS=TRUE){
  ...
  for(i in seq_along(ParkUnits)){
    poly<-.get_unit_polygon(ParkUnits[i])
...
#' @examples
#'  \dontrun{
#' set_cui(emlObject, "PUBFUL")
#' }
set_cui<-function(emlObject, CUIcode=c("PUBFUL", "PUBVER", "NOCON", "DL ONLY", "FEDCON", "FED ONLY", "NPSONLY"), NPS=TRUE){
...
#' set_drr_doi(emlObject, "2293234", DRRtitle)
#' }
set_drr_doi<-function(emlObject, DRRrefID, DRRtitle, NPS=TRUE){
  if(NPS==TRUE){org<-"NPS"}
  else{org<-NPS}
...
#' @examples
#'  \dontrun{
#' emlObject<-set_abstract(emlObject, "This is a very short abstract")
#' }
set_abstract<-function(emlObject, abstract, NPS=TRUE){
...
#' @examples
#'  \dontrun{
#' emlObject<-litcited2<-set_lit(emlObject, "bibfile.bib")
#' }
set_lit<-function(emlObject, bibtex_file, NPS=TRUE){
...
#' }
set_language<-function(emlObject, lang, NPS=TRUE){
...
#' @examples
#' \dontrun{
#' set_protocol(emlObject, 2222140)
#' }
#'
set_protocol<-function(emlObject, protocolID, NPS=TRUE){
