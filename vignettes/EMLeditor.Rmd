...
1) Assemble your data as one or more .csv files, saved in UTF-8 format, in a single directory without any other .csv files.
2) Use the [EML template script](a03_EML_creation_script.html) to guide you while supplying the information necessary to construct an initial EML file. The functions will generate multiple .txt files, which you will need to manually edit either in a text editor (e.g. notepad) or spreadsheet program (e.g. MS Excel).
3) Use the `EMLassemblyline::make_eml()` to generate an initial EML document.
4) Following the EML script, use EMLeditor functions to edit the metadata in R and evaluate whether your metadata is acceptable (don't forget to use `EML::eml_validate()` to make sure you are generating valid EML).
...
5) Upload to DataStore (via web or `EMLeditor::uplad_data_package()`).
...
Make sure all your data files (.csv) and your metadata (*_metadata.xml) are in a single folder. You can then use the [DPchecker package](https://nationalparkservice.github.io/DPchecker/) to run automated checks on your data package. DPchecker should have loaded when you loaded the NPSdataverse:

```{r check_data_package, eval=FALSE}
#path to your data package:
dir <- here::here("..", "..", "data_packages", "my_current_package")
run_congruence_checks(dir)
```

Any errors (a red X) should be addressed before moving forward. Warnings (a yellow !) are worth looking at, but may not need to be fixed. For example, if your data package does not have any taxonomy, this will throw a warning (it's unusual for most IMD data packages) but may be completely fine to leave as-is. If you do find warnings, there is often an additional EMLeditor function to help you resolve the issue (see "Additional functions" below)

### Upload your data package

If your data package has a DOI, EMLeditor can assist with uploading it to the correct draft reference on DataStore. You'll need to be logged in via VPN for this part, and it will only work if each individual file is <32Mb.

```{r upload_package, eval=FALSE}
#path to your package:
dir <- here::here("..", "..", "data_pacakges", "my_current_package")
upload_datapackage(dir)
```
...
### Load your .xml into R:

```{r load_metadata, eval=FALSE}
```{r set_title, eval=FALSE}
#path to your data package:
dir <- here::here("..", "..", "data_packges", "my_current_package")

#load data metadata (via DPchecker):
my_meta2 <- load_metadata(dir)
```
...
#don't forget to write your metadata back to .xml!
#(this will save your .xml to your working directory - don't forget to replace your old metadata file in your data package folder)
EML::write_eml(my_meta2, "my_new_metadata.xml"
```

For a full list of available EMLeditor functions, see the [EMLeditor reference page](../reference/index.html).
