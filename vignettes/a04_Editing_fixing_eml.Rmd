...
#assuming your metadata file is in another directory:
...
You can add an individual or an organization as an author ("creator") and then re-order the authors to reflect whatever order you would like them to appear in the citation.

First, view the current authors ("creators"). Note that this function will does not yet work well if there is already an organization as an author:
...
# Set a park unit as an author:
metadata <- set_creator_orgs(metadata, park_units = "IMD")

# Set an outside organization as an author:
metadata <- set_creator_orgs(metadata, creator_orgs = "Broadleaf, Inc.")

# Optionally, supply a custom ROR (Reasearch Organization Registry) ID if you have one:
metadata <- set_creator_orgs(metadata, park_units = "YELL", RORs = "04rw5yk74")
```

Any author or creator you add will be added to the end of the list of authors/creators. You may then wish to re-order the authors/creators:
